...
- 概览和工作台按当前业务凭证加载可调用模型；资源管理使用完整管理目录，同名模型通过 Provider 前缀区分。首次配置提供认证、凭证与测试入口，创建 Key 后可直接用于当前工作台。
- 工作台支持可选的最大输出 Token，留空不添加预算限制；模型明确提供时会显示上下文窗口和输出上限。响应展示结束原因、耗时、首个输出时间及 Request ID，并区分输出截断、内容过滤与请求失败。
- 页面切换保留工作台草稿、参数和响应，进行中的生成继续运行。内容仅保留在当前页面会话内；刷新、完全退出后重新启动或切换访问凭证会清空，切换凭证也会中止旧请求。
- 从工作台可直接定位请求详情。请求明细支持 `requestId` 和 `finishReason` 精确筛选，作用于保留明细并在分页前应用，不改变汇总和趋势，也不会绕过 Key 访问范围。
- 服务设置读取失败时显示重试入口并禁止保存；成功加载后，仅在配置有变更且校验通过时允许保存并重启。
