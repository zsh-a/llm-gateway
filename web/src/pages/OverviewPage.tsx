  ...
  return (
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/20 via-card to-cyan-500/5 p-6 panel-shadow">
        ...
        <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div className="max-w-2xl">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/25 bg-primary/10 px-2.5 py-1 text-[11px] text-primary"><Zap className="size-3" />OpenAI-compatible LLM Gateway</div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard icon={TrendingUp} label="成功率" value={summary.successRate === null ? "—" : Number(summary.successRate).toFixed(1) + "%"} detail={formatNumber(summary.errors) + " 个错误"} tone="green" />
        ...
        <StatCard icon={Database} label="Token 用量" value={formatCompact(summary.tokens.totalTokens)} detail={formatCompact(summary.tokens.cachedTokens) + " cached · " + formatCompact(summary.tokens.reasoningTokens) + " reasoning"} tone="amber" />
      </div>

      <div className="grid gap-6 xl:grid-cols-[1.45fr_0.85fr]">
        <Card>
          <CardHeader className="flex-row items-start justify-between">
            <div><h3 className="text-sm font-semibold tracking-tight">请求趋势</h3><CardDescription>过去 24 小时的请求和 Token 活跃度</CardDescription></div>
            <Button variant="ghost" size="sm" onClick={() => onNavigate("metrics")}>查看详情<ArrowUpRight className="size-3.5" /></Button>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between rounded-xl border border-border/60 bg-muted/20 px-3 py-2.5"><div className="flex items-center gap-2 text-xs text-muted-foreground"><Activity className="size-3.5 text-primary" />请求活跃度</div><Sparkline values={requestValues} /></div>
            <div className="mt-4"><MetricChart points={data.timeseries} /></div>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><h3 className="text-sm font-semibold tracking-tight">认证状态</h3><CardDescription>认证缓存可被多个 Provider 渠道复用</CardDescription></CardHeader>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Card>
          <CardHeader className="flex-row items-start justify-between"><div><h3 className="text-sm font-semibold tracking-tight">模型目录</h3><CardDescription>当前 Gateway 可路由模型</CardDescription></div><Badge variant="info">{data.models.length} models</Badge></CardHeader>
        ...
        <Card>
          <CardHeader className="flex-row items-start justify-between"><div><h3 className="text-sm font-semibold tracking-tight">最近请求</h3><CardDescription>最新完成的调用记录</CardDescription></div><Button variant="ghost" size="sm" onClick={() => onNavigate("metrics")}>全部记录<ChevronRight className="size-3.5" /></Button></CardHeader>
